    ...
    class TGM_Plugin_Activation {
        ...
        public function __construct() {
            ...
            $this->strings = array(
                ...
                'oops'                           => __( 'Something went wrong.', 'starhotel' ),
                'notice_can_install_required'    => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'starhotel' ),
                'notice_can_install_recommended' => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'starhotel' ),
                'notice_cannot_install'          => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'starhotel' ),
                'notice_can_activate_required'   => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'starhotel' ),
                'notice_can_activate_recommended'=> _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'starhotel' ),
                'notice_cannot_activate'         => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'starhotel' ),
                'notice_ask_to_update'           => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'starhotel' ),
                'notice_cannot_update'           => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'starhotel' ),
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' == $_GET['tgmpa-install'] ) ) {
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . esc_url( network_admin_url() ) . '" title="' . esc_attr__( 'Return to the Dashboard', 'starhotel' ) . '">' . __( 'Return to the Dashboard', 'starhotel' ) . '</a>' ) . '</p>';
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'starhotel' ), $imploded, $count ) . '</p>';
                }

                // Setup variables to determine if action links are needed.
                $show_install_link  = $install_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), network_admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'starhotel' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . esc_url( add_query_arg( 'page', urlencode( $this->menu ), network_admin_url( 'themes.php' ) ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'starhotel' ) . '</a>'  : '';
                ...
                if ( ! empty( $this->strings['nag_type'] ) ) {
                    add_settings_error( 'starhotel', 'starhotel', $rendered, sanitize_html_class( strtolower( $this->strings['nag_type'] ) ) );
                } else {
                    add_settings_error( 'starhotel', 'starhotel', $rendered, $nag_class );
                }
            }
    ...
    class TGMPA_List_Table extends WP_List_Table {
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'starhotel' );
                    // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'starhotel' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'starhotel' );
                }

                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'starhotel' ) : __( 'Recommended', 'starhotel' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'starhotel' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'starhotel' ) );
                }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . esc_attr__( 'Install', 'starhotel' ) . ' %2$s">' . __( 'Install', 'starhotel' ) . '</a>',
                        esc_url(
						    wp_nonce_url(
                                add_query_arg(
                                    array(
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . esc_attr__( 'Activate', 'starhotel' ) . ' %2$s">' . __( 'Activate', 'starhotel' ) . '</a>',
                        esc_url(
                            add_query_arg(
                                array(
                                    'page'                 => urlencode( TGM_Plugin_Activation::$instance->menu ),
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'starhotel' ), network_admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'tgmpa-bulk-install'  => __( 'Install', 'starhotel' ),
                'tgmpa-bulk-activate' => __( 'Activate', 'starhotel' ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'starhotel' ), $imploded );
                }
    ...
    if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
        ...
        class TGM_Bulk_Installer extends WP_Upgrader {
            ...
            public function install_strings() {
                ...
                $this->strings['no_package']          = __( 'Install package not available.', 'starhotel' );
                $this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'starhotel' );
                $this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'starhotel' );
                $this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'starhotel' );
                $this->strings['process_failed']      = __( 'Plugin install failed.', 'starhotel' );
                $this->strings['process_success']     = __( 'Plugin installed successfully.', 'starhotel' );
        ...
        class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
            ...
            public function add_strings() {
                ...
                if ( TGM_Plugin_Activation::$instance->is_automatic ) {
                    ...
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'starhotel' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'starhotel' ) . '</span><span class="hidden">' . __( 'Hide Details', 'starhotel' ) . '</span>.</a>';
                ...
                // Default installation strings.
                    ...
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'starhotel' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'starhotel' ) . '</span><span class="hidden">' . __( 'Hide Details', 'starhotel' ) . '</span>.</a>';
            ...
            public function bulk_footer() {
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . esc_url( network_admin_url() ) . '" title="' . esc_attr__( 'Return to the Dashboard', 'starhotel' ) . '">' . __( 'Return to the Dashboard', 'starhotel' ) . '</a>' ) . '</p>';
                    echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                }
