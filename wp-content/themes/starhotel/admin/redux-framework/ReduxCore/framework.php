        ...
        class ReduxFramework {
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( empty ( $this->args['footer_credit'] ) ) {
                    $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( __( 'Options panel created using %1$s', 'redux-framework-starhotel' ), '<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . __( 'Redux Framework', 'redux-framework-starhotel' ) . '</a> v' . self::$_version ) . '</span>';
                }

                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = __( 'Options', 'redux-framework-starhotel' );
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = __( 'Options', 'redux-framework-starhotel' );
                }
...
// __construct()
           ...
           public function vc_fixes() {
            ...
            private function _internationalization() {
                 ...
                 * @param string     'redux-framework-starhotel'  text domain
                 */
                $locale = apply_filters( "redux/textdomain/{$this->args['opt_name']}", get_locale(), 'redux-framework-starhotel' );
                ...
                load_textdomain( 'redux-framework-starhotel', self::$_dir . 'languages/' . $locale . '.mo' );
            }
                ...
                if ( $this->args['menu_type'] == 'hidden' ) {
                    ...
                    if ( true === $this->args['allow_sub_menu'] ) {
                        ...
                        if ( true == $this->args['system_info'] ) {
                            add_submenu_page(
                                $this->args['page_slug'], __( 'System Info', 'redux-framework-starhotel' ), __( 'System Info', 'redux-framework-starhotel' ), $this->args['page_permissions'], $this->args['page_slug'] . '&tab=system_info_default', '__return_null'
                            );
                        }
                    }
                }
...
// _enqueue()
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $dismiss = 'true';
                    $s       = __( 'Enable', 'redux-framework-starhotel' );

                    // Values for disabling hints.
                    if ( 'true' == $hint_status ) {
                        $dismiss = 'false';
                        $s       = __( 'Disable', 'redux-framework-starhotel' );
                    }

                    // Make URL
                    $url = '<a class="redux_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

                    $event = __( 'moving the mouse over', 'redux-framework-starhotel' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = __( 'clicking', 'redux-framework-starhotel' );
                    }

                    // Construct message
                    $msg = sprintf( __( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'redux-framework-starhotel' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'redux-hint-tab',
                        'title'   => __( 'Hints', 'redux-framework-starhotel' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
...
// admin_footer_text()
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = __( 'Default', 'redux-framework-starhotel' ) . ": " . substr( $default_output, 0, - 2 );
                }
...
//                }
            ...
            public function ajax_save() {
                ...
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" ) ) {
                    json_encode( array(
                        'status' => __( 'Invalid security credential, please reload the page and try again.', 'redux-framework-starhotel' ),
                        'action' => 'reload'
                    ) );
                    die();
                }
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( ! empty ( $values ) ) {
                        ...
                        echo json_encode( array( 'status' => __( 'Your panel has no fields. Nothing to save.', 'redux-framework-starhotel' ) ) );
                    }
                }
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
