...
You can call the client at the following phone number: ", 'starhotel') . "$phone" ; } else {}      
    ...
    // Mail content
    $email_content = esc_html__('Good news! A reservation has been requested by ', 'starhotel') . "$email". esc_html__('
The customer wants to check-in at: ', 'starhotel') . "$checkin". esc_html__('
and check-out at: ', 'starhotel') . "$checkout". esc_html__('

The customer requested a  ', 'starhotel') . "$room". esc_html__(' room for  ', 'starhotel') . "$adults". esc_html__(' adult(s) and  ', 'starhotel') . "$children". esc_html__(' child(ren).
You can contact the customer via email, ', 'starhotel') . "$email" . esc_html__(' or hit reply in your email client to make the reservation complete.', 'starhotel') . "$phone_content"; 
	...
	$SMTPMail = new SMTPClient ($SmtpServer, $SmtpPort, $SmtpUser, $SmtpPass, $email, $recipient, $subject, $email_content, $email_headers);
	
	// Main messages
	if ($SMTPMail->SendMail())
    {
        echo '<h1>' . esc_html__('Reservation sent successfully!', 'starhotel' ) . '</h1>';
        echo '<p>' . esc_html__('Thank you, your reservation has been submitted to us and we will contact you as quickly as possible to complete your booking.', 'starhotel' ) . '</p>';
    }
    else
    {
        echo '<p>' . esc_html__('Oops! Something went wrong and we could not send your reservation.', 'starhotel' ) . '</p>';
    }
}
else
{
